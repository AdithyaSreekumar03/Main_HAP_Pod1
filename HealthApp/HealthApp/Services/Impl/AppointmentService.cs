...
namespace HealthApp.Service.Impl
{
    public class AppointmentService : IAppointmentService
    {
            ...
            string slot)
        {
            ...
            }// ✅ FIXED: culture-safe parsingif (date.Date == DateTime.Today)
            {
        ...
        public void CancelAppointment(int appointmentId, string reason)
        {
            ...
            {
                throw new AppointmentAlreadyCompletedException("Completed appointment cannot be cancelled.");
            }
        ...
        {
            return _repo.GetAll().Where(a =>
                    a.Patient.PatientId == patientId &&
                    (a.Status == AppointmentStatus.Pending ||
                     a.Status == AppointmentStatus.Confirmed) &&
                    a.ScheduledDate.Date >= DateTime.Today).OrderBy(a => a.ScheduledDate).ThenBy(a => a.TimeSlot).ToList();
        }
            ...
            DateTime toDate)
        {
            ...
            return _repo.GetAll().Where(a =>
                    ...
                    a.ScheduledDate.Date <= toDate.Date).OrderBy(a => a.ScheduledDate).ThenBy(a => a.TimeSlot).ToList();
        }
        public List<Appointment> GetPendingAppointmentsByDoctor(int doctorId)
        {
            return _repo.GetAll().Where(a =>
                    a.Doctor.DoctorId == doctorId &&
                    a.Status == AppointmentStatus.Pending).OrderBy(a => a.ScheduledDate).ThenBy(a => a.TimeSlot).ToList();
        }
        public List<string> CheckDoctorAvailability(int doctorId, DateTime date)
        {
            ...
            var bookedSlots = _repo.GetAll().Where(a =>
                    a.Doctor.DoctorId == doctorId &&
                    a.ScheduledDate.Date == date.Date &&
                    a.Status != AppointmentStatus.Cancelled).Select(a => a.TimeSlot.ToUpper().Trim()).ToList();

            var availableSlots = TimeSlots.Slots
                .Where(s => !bookedSlots.Contains(s.ToUpper().Trim())).ToList();
